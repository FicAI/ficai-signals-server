...
source test.env
export FICAI_LISTEN FICAI_DB_HOST FICAI_DB_PORT FICAI_DB_USERNAME FICAI_DB_PASSWORD FICAI_DB_DATABASE

TEST_UID=7357
TEST_URL="https://forums.sufficientvelocity.com/threads/on-fluttering-wings-taylor-hebert-in-mgln.53204/"

request_get() {
  curl -s -f -D "$SHUNIT_TMPDIR/headers" -o "$SHUNIT_TMPDIR/out" \
...
extractSignal() {
  <"$SHUNIT_TMPDIR/out" jq -r ".[]|select(.tag==\"$1\")"
}
...
oneTimeSetUp() {
  ...
  request_patch "$TEST_UID" "$TEST_URL" "%worm" "%taylor hebert" "%taylor"
}
...
test404() {
  curl -s -f -D "$SHUNIT_TMPDIR/headers" -o "$SHUNIT_TMPDIR/out" "http://$FICAI_LISTEN/derp"
...
testGetEmptySignals() {
  ...
  assertEquals '[]' "$(cat "$SHUNIT_TMPDIR/out")"
}
